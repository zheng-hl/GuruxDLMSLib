...
{
	vector<pair<CGXDateTime, CGXDateTime> > m_ListeningWindow;
	vector<string> m_AllowedSenders;
	vector<std::pair<string, std::pair<int, CGXDLMSScriptAction> > > m_SendersAndActions;
public:	
	//Constructor.
	...
	//LN Constructor.
	CGXDLMSMessageHandler(basic_string<char> ln);

	/** 
    Listening Window.
    */
    vector<pair<CGXDateTime, CGXDateTime> >& GetListeningWindow();

   /** 
    List of allowed Senders.
   */
    vector<string>& GetAllowedSenders();
   ...
   */
	vector<std::pair<string, std::pair<int, CGXDLMSScriptAction> > >& GetSendersAndActions();
