...
int CGXStandardObisCodeCollection::GetBytes(basic_string<char> ln, unsigned char* bytes)
{
	vector< basic_string<char> > tmp = GXHelpers::Split(ln, '.');
	if (tmp.size() != 6)
    {
        //Invalid OBIS Code.
...
bool CGXStandardObisCodeCollection::EqualsInterface(CGXStandardObisCode item, int ic)
{
	...
	sprintf(type, "%d", ic);
#endif				
	vector< basic_string<char> > tmp = GXHelpers::Split(item.GetInterfaces(), ',');
...
// Check OBIS code.    
bool CGXStandardObisCodeCollection::EqualsObisCode(vector< basic_string<char> >& obisMask, unsigned char* ic)
{
...
{		
	char buff[6];
	for(vector<CGXStandardObisCode>::iterator it = this->begin(); it != this->end(); ++it)
    {
        ...
        if (EqualsInterface(*it, IC) && EqualsObisCode((*it).GetOBIS(), pObisCode))
        {
            ...
            vector< basic_string<char> > tmp2 = GXHelpers::Split((*it).GetDescription(), ';');		
            if (tmp2.size() > 1)
            {
                basic_string<char> desc = GetDescription(tmp2[1]);
                if (desc != "")
                {
                    tmp2[1] = desc;
                    basic_string<char> builder;
					for(vector< basic_string<char> >::iterator s = tmp2.begin(); s != tmp2.end(); ++s)
					{
						if (builder.size() != 0)
                        {
                            builder.append(";");
            ...
            vector< basic_string<char> > obis = tmp.GetOBIS();				
#if _MSC_VER > 1000
            ...
            obis[5] = buff;
            tmp.SetOBIS(obis);
            basic_string<char> desc = tmp.GetDescription();
#if _MSC_VER > 1000
			sprintf_s(buff, 6, "%d", pObisCode[0]);
#else
			sprintf(buff, "%d", pObisCode[0]);
